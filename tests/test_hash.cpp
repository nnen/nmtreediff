...
Tree parse(const nmtreediff::IFormatProvider& provider, const std::string& xml) {
...
std::uint64_t rootHash(const nmtreediff::IFormatProvider& provider, const std::string& xml) {
...
TEST_CASE("an empty record, an empty sequence and an empty scalar are three properties",
    ...
    nmtreediff::Property scalar;
    scalar.name = "tags";
    nmtreediff::Property record = scalar;
    record.form = nmtreediff::PropertyForm::Record;
    nmtreediff::Property sequence = scalar;
    sequence.form = nmtreediff::PropertyForm::Sequence;

    CHECK(nmtreediff::hashProperty(scalar) != nmtreediff::hashProperty(record));
    CHECK(nmtreediff::hashProperty(scalar) != nmtreediff::hashProperty(sequence));
    CHECK(nmtreediff::hashProperty(record) != nmtreediff::hashProperty(sequence));
}
...
TEST_CASE("a record's value is part of its hash", "[hash][form]") {
    ...
    nmtreediff::Property before;
    before.name = "speed";
    before.value = "1.0";
    before.form = nmtreediff::PropertyForm::Record;
    before.children.push_back(part);

    nmtreediff::Property after = before;
    after.value = "2.0";

    CHECK(nmtreediff::hashProperty(before) != nmtreediff::hashProperty(after));
}

TEST_CASE("a record's parts hash as a set and a sequence's in order", "[hash][form]") {
    nmtreediff::Property a;
    a.name = "a";
    a.value = "1";
    nmtreediff::Property b;
    b.name = "b";
    b.value = "2";

    nmtreediff::Property record;
    record.name = "p";
    record.form = nmtreediff::PropertyForm::Record;
    record.children = {a, b};
    nmtreediff::Property recordSwapped = record;
    recordSwapped.children = {b, a};
    CHECK(nmtreediff::hashProperty(record) == nmtreediff::hashProperty(recordSwapped));

    nmtreediff::Property sequence = record;
    sequence.form = nmtreediff::PropertyForm::Sequence;
    nmtreediff::Property sequenceSwapped = recordSwapped;
    sequenceSwapped.form = nmtreediff::PropertyForm::Sequence;
    CHECK(nmtreediff::hashProperty(sequence) != nmtreediff::hashProperty(sequenceSwapped));
}
...
TEST_CASE("a property nested thousands of levels deep hashes without recursing", "[hash][deep]") {
    ...
    nmtreediff::Property deep;
    deep.name = "leaf";
    deep.value = "x";
    for (int level = 0; level < kDepth; ++level) {
        nmtreediff::Property wrapper;
        wrapper.name = "wrap";
        wrapper.form = nmtreediff::PropertyForm::Record;
        wrapper.children.push_back(std::move(deep));
        deep = std::move(wrapper);
    }

    const std::uint64_t h = nmtreediff::hashProperty(deep);
    CHECK(h != 0);
    CHECK(h == nmtreediff::hashProperty(deep));
}
